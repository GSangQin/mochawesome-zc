...
function Mochawesome(runner, options) {
  ...
  // 兼容mocha-parallel-tests不解析reporterOptions
  var reporterOptions = options.reporterOptions;
  if (reporterOptions && typeof reporterOptions === 'string') {
    var newReporterOptions = {};
    reporterOptions.replace(/([^=]+)=([^,]*)/g, function (all, key, value) {
      newReporterOptions[key] = value;
    });
    reporterOptions = newReporterOptions;
  }

  // Create/Save necessary report dirs/files
  var reporterOpts = reporterOptions || {},
  ...
  runner.on('suite end', function (suite) {
    if (suite && suite.tests.length > 0) {
      arrSuite.push(suite);
    } else if (suite && suite.title.length > 0) {
      suite.suites = arrSuite;
  ...
  runner.on('end', function () {
    try {
      if (!endCalled) {
		...
		obj.stats.suites = obj.suites.suites.length;
...
function cleanSuite(suite, config) {
  ...
  if (suite && (!config.copyShotOnlyFail || suite.hasFailures)) {
    copyScreenShots(suite.title, suite.tests);
  }
  ...
  function copyScreenShots(title, allTests) {
    var test;
    var HOST_REG = /^https?:\/\/(.*)(\.html?)/;

    for (var i = 0; i < allTests.length; i++) {
      test = allTests[i];
      var screenshotPath = '_' + (i + 1);

      var match = HOST_REG.exec(title);
      if (match) {
        screenshotPath = match[1].split('/').join('-') + screenshotPath;
      } else {
        screenshotPath = title.split('//')[1].split('/').join('-') + screenshotPath;
      }
